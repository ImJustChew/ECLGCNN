...
def cross_validation(data, labels, seed):
    ...
    for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(len(data)))):
        ...
        model = ECLGCNN(K=params['K'], T=params['T'], num_cells=params['num_cells']).to(device)
...
def subject_independent_exp():
    print('------------------------------------------------')
    print('|        subject independent experiment        |')
    print('------------------------------------------------')

    params['K'] = 10
    params['num_cells'] = 150
    params['e'] = 0.12
    params['lr'] = 0.003
    params['alpha'] = 0.00008

    dataset = DeapDataset('./data')
    avg_acc_list = []
    avg_f_score_list = []

    for i in range(3): # 3 labels: valence, arousal, dominance
        ...
        for seed in seeds:
            ...
            for step, (data, label) in enumerate(dataset, start=1):
                used_label = label[:, i]
                print(f'load: seed {seed}, subject {step}: samples: {len(data)}, labels: {used_label.shape}')
                combined_data = combined_data + data
                combined_labels.append(used_label)

            combined_labels = torch.cat(combined_labels, dim=0).to(device)
            print(f'combined_data: {len(combined_data)}, combined_labels: {len(combined_labels)}')

            model_path = model_dir + f'seed{seed}/model_{label_types[i]}.pt'
            if os.path.exists(model_path):
                print('model already exists, loading...')
                trained_models, acc, f_score = torch.load(model_path)
                print('model loaded.')
            else:
                trained_models, acc, f_score = cross_validation(combined_data, combined_labels, seed)
